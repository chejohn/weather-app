...
var getGeoData = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(defaultLocation) {
    var locationData, responseObj, locationObj, city, latitude, longitude;
...
var getWeatherData = /*#__PURE__*/function () {
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 5:
            ...
            return _context2.abrupt("return", weatherObj);
  ...
  return function getWeatherData(_x2, _x3) {
...
var convertToLocalTime = function convertToLocalTime(unixTime) {
  ...
  var militaryHour = dateObj.getHours();
  var minutes;
  ...
  if (militaryHour > 12) {
    if (appendMinutes) {
      normalTime = "".concat(militaryHour - 12, ":").concat(minutes, " pm");
    } else {
      normalTime = "".concat(militaryHour - 12, " pm");
    }
  } else if (militaryHour === 12) {
    if (appendMinutes) {
      normalTime = "12:".concat(minutes, " pm");
    } else {
      normalTime = "12 pm";
    }
  } else if (militaryHour === 0) {
    if (appendMinutes) {
      normalTime = "12:".concat(minutes, " am");
    } else {
      normalTime = "12 am";
    }
  } else {
    if (appendMinutes) {
      normalTime = "".concat(militaryHour, ":").concat(minutes, " am");
    } else {
      normalTime = "".concat(militaryHour, " am");
    }
  }
...
var convertToDay = function convertToDay(unixTime) {
...
var convertToPercent = function convertToPercent(number) {
  return "".concat(Math.floor(number * 100), "%");
};

var convertToWindVector = function convertToWindVector(windSpeed, windDegrees) {
  var direction;
  var newWindSpeed = Math.floor(windSpeed);
  ...
  return "".concat(direction, " ").concat(newWindSpeed, " mph");
...
var toVisibilityInMiles = function toVisibilityInMiles(meters) {
  return "".concat(Math.floor(meters / 1610), " m");
};

var filterForCurrentData = function filterForCurrentData(weatherObj) {
  var currentData = weatherObj.current;
  var precipitation;
  var sunrise = convertToLocalTime(currentData.sunrise, true);
  var sunset = convertToLocalTime(currentData.sunset, true);
  var humidity = "".concat(currentData.humidity, "%");
  var windVector = convertToWindVector(currentData.wind_speed, currentData.wind_deg);
  ...
  var pressure = "".concat(currentData.pressure, " hPa");
  var visibility = toVisibilityInMiles(currentData.visibility);
  var uvIndex = "".concat(currentData.uvi);
  var temp = "".concat(Math.floor(currentData.temp), "&deg;");
  var weatherState = currentData.weather[0].main;
  return {
    sunrise: sunrise,
    sunset: sunset,
    humidity: humidity,
...
var filterForHourlyData = function filterForHourlyData(iteration, weatherObj) {
  var hourlyData = weatherObj.hourly[iteration];
  var time = convertToLocalTime(hourlyData.dt);
  var chanceOfRain = convertToPercent(hourlyData.pop);
  var temp = "".concat(Math.floor(hourlyData.temp), "&deg;");
  var weatherIcon = hourlyData.weather[0].icon;
  return {
    time: time,
...
var filterForDailyData = function filterForDailyData(iteration, weatherObj) {
  var dailyData = weatherObj.daily[iteration];
  var nightTemp;
  var weatherState;

  if (iteration === 0) {
    nightTemp = "".concat(Math.floor(dailyData.temp.night), "&deg;");
    weatherState = dailyData.weather[0].description;
  }

  var chanceOfRain = convertToPercent(dailyData.pop);
  var highTemp = "".concat(Math.floor(dailyData.temp.max), "&deg;");
  var lowTemp = "".concat(Math.floor(dailyData.temp.min), "&deg;");
  var humidity = "".concat(dailyData.humidity, "%");
  var weatherIcon = dailyData.weather[0].icon;
  var day = convertToDay(dailyData.dt);
  return {
    chanceOfRain: chanceOfRain,
    highTemp: highTemp,
    lowTemp: lowTemp,
...
var updateCurrentComponent = function updateCurrentComponent(weatherObj, location) {
  ...
  var currentInterface2Arr = GlobalNodes.currentInterface2;
...
var updateHourlyComponent = function updateHourlyComponent(weatherObj) {
  var hourlyInterfaceArr = GlobalNodes.hourlyInterface;
  ...
  for (var i = 0; i < hourlyInterfaceArr.length; i++) {
    ...
    hourlyEntry.children[2].src = "https://openweathermap.org/img/wn/".concat(HourlyData.weatherIcon, "@4x.png");
...
var updateDailyComponent = function updateDailyComponent(weatherObj) {
  for (var i = 0; i < 8; i++) {
    ...
    if (i === 0) {
      ...
      GlobalNodes.currentInterface1.children[3].innerHTML = "H:".concat(highTemp, " L:").concat(DailyData.lowTemp);
      GlobalNodes.weatherDescription.innerHTML = "Today: ".concat(DailyData.weatherState, ". The high will be ").concat(highTemp, ". The low tonight will be ").concat(DailyData.nightTemp, ".");
      GlobalNodes.currentInterface2[2].textContent = "".concat(DailyData.chanceOfRain);
    } else {
      var currentDay = GlobalNodes.daysOfTheWeek[i - 1];
      currentDay.children[0].textContent = DailyData.day;
      currentDay.children[1].src = "https://openweathermap.org/img/wn/".concat(DailyData.weatherIcon, "@4x.png");
      ...
      GlobalNodes.weekTemp[i - 1].innerHTML = "".concat(DailyData.highTemp, " ").concat(DailyData.lowTemp);
    }
  }
};
...
var GlobalNodes = function () {
  var input = document.querySelector('input');
  var searchBttn = document.querySelector('.search-bttn');
  var currentInterface1 = document.querySelector('.current-interface-1');
  var currentInterface2 = document.querySelectorAll('.current-interface-2');
  var hourlyInterface = document.querySelectorAll('.hourly-status-panel');
  var daysOfTheWeek = document.querySelectorAll('.week-status-flex');
  var weekChanceOfRain = document.querySelectorAll('#chance-of-rain');
  var weekHumidity = document.querySelectorAll('#humidity');
  var weekTemp = document.querySelectorAll('.temp');
  var weatherDescription = document.querySelector('.current-weather-statement');
  ...
  return {
    ...
    currentInterface1: currentInterface1,
    currentInterface2: currentInterface2,
    hourlyInterface: hourlyInterface,
    daysOfTheWeek: daysOfTheWeek,
    weekChanceOfRain: weekChanceOfRain,
